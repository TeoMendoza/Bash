...
pub fn pipe_ramp_2_collider() -> ComplexCollider {
    let pipe_ramp_2_convex_hull_0: ConvexHullCollider = create_convex_hull_collider(PIPE_RAMP_2_CONVEX_HULL0_VERTICES, PIPE_RAMP_2_CONVEX_HULL0_TRIANGLE_INDICES_LOCAL, 0.0, ConvexHullColliderType::None);
    let plane_convex_hulls: Vec<ConvexHullCollider> = vec![pipe_ramp_2_convex_hull_0];
    create_complex_collider(plane_convex_hulls, DbVector3 { x: 0.0, y: 5.5, z: 15.625 })
}
