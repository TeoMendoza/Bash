...
pub fn right_ramp_2_collider() -> ComplexCollider {
    let right_ramp_2_convex_hull_0: ConvexHullCollider = create_convex_hull_collider(RIGHT_RAMP_2_CONVEX_HULL0_VERTICES, RIGHT_RAMP_2_CONVEX_HULL0_TRIANGLE_INDICES_LOCAL, 0.0, ConvexHullColliderType::None);
    let plane_convex_hulls: Vec<ConvexHullCollider> = vec![right_ramp_2_convex_hull_0];
    create_complex_collider(plane_convex_hulls, DbVector3 { x: 8.2, y: 2.0, z: -20.0 })
}
